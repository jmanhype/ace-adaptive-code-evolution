defmodule Ace.Core.EvolutionProposal do
  ...
  ## Parameters
    ...
    - `status`: Status to filter by ("pending_review", "approved", "rejected", "applied")
    - `opts`: Optional parameters
      - `:limit` - Maximum number of proposals to return
      - `:offset` - Number of proposals to skip
  ...
  @doc """
  Gets counts of proposals by status.
  
  ## Returns
  
    - Map with counts for each status
  """
  def count_by_status() do
    statuses = ["pending_review", "approved", "rejected", "applied"]
    
    Enum.reduce(statuses, %{}, fn status, acc ->
      count = from(p in __MODULE__, where: p.status == ^status, select: count(p.id))
              |> Ace.Repo.one()
